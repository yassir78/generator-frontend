...
export class RoleListComponent implements OnInit {
  ...
  deleteMenuRole(menuRole: MenuRole){
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.selectedRole.menuRoles = this.selectedRole.menuRoles.filter(val => val.menu.libelle !== menuRole.menu.libelle);
        this.messageService.add({severity: 'success', summary: 'Successful', detail: 'Menu '+menuRole.menu.libelle+' deleted successfully!', life: 3000});
      }
    });
  }
